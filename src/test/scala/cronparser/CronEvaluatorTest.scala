...
class CronEvaluatorTest extends FlatSpec with Matchers {
  ...
  "a letter in any position" should "return an empty list" in {
    val cronPositions = List(Minute, Hour, DayOfMonth, Month, DayOfWeek)
    cronPositions.flatMap(cronExpressionEvaluation("hello", _).fromToToList) shouldEqual List()
  }
}
